...
function getSiteInfo({ collection, block, NOTION_CONFIG }) {
  const defaultTitle = NOTION_CONFIG?.TITLE || '0xc0de'
  const defaultDescription =
    NOTION_CONFIG?.DESCRIPTION || 'Frank Chang\'s blog'
  const defaultPageCover = NOTION_CONFIG?.HOME_BANNER_IMAGE || ''
  const defaultIcon = NOTION_CONFIG?.AVATAR || ''
